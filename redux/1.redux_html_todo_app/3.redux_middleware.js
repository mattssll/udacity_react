...
// App code
// Map of our Actions - each is gonna do a different thing somehow.
const ADD_TODO = 'ADD_TODO'
const REMOVE_TODO = 'REMOVE_TODO'
const TOGGLE_TODO = 'TOGGLE_TODO'
const ADD_GOAL = 'ADD_GOAL'
const REMOVE_GOAL = 'REMOVE_GOAL'
// Defining the actions that are implementing our Actions
...
const checker = (store) => (next) => (action) => { // this is like calling a next function inside store that calls another one isnide of it (action)
    if (action.type === ADD_TODO && action.todo.name.toLowerCase().includes('bitcoin')) 
            ...
            {
                return alert ("Nope, bitcoind is a bad idea");
            }
            else {
                return next(action); // next is our dispatcher or our next middleware
            }
}
...
const store = Redux.createStore(Redux.combineReducers({
    todos,
    goals, // store will have 3 methods: getState, subscribe, dispatch
}), Redux.applyMiddleware(checker)); // checker is our middleware
