...
function createStore (reducer) {
    ...
    const subscribe = (listener) => {
        ...
        return () => { // also returns a fx that could be used by our user to unsubscribe 
            listeners = listeners.filter((l) => l !== listener)
        }
    }

    const dispatch = (action) => {
        state = reducer(state, action) // pass current state and action and get new state
...
function app (state = {}, action) { // root reducer
    return {
...
const store = createStore(app); // store will have 3 methods
